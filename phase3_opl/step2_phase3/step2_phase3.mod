...
*********************************************/
 ...
 float ramp_rate = ...; //ramp rate of generators
 ...
 float alpha_raw[load][time] = ...; //price elasticity of load
 float eur_to_dkk = 7.444; //convert alpha from Euro to DKK
 float alpha[l in load][t in time] = eur_to_dkk*alpha_raw[l][t]/consumption_split_ratio[l]; // readjust alpha for different loads
 
  // values from WP6 until Sintef provides them
 float changel_max_raw[l in load][t in time] = p_ref_raw[l][t]*0.1; //what is max flexibility in MW
 float changel_min_raw[l in load][t in time] = -p_ref_raw[l][t]*0.1; //what is min flexibility in MW
 ...
 float changel_max[l in load][t in time] = changel_max_raw[l][t]*flex_load[l]*scaling_factor; //what is max flexibility in MW
 float changel_min[l in load][t in time] = changel_min_raw[l][t]*flex_load[l]*scaling_factor; //what is min flexibility in MW
 ...
 float max_price[t in time] = abs(spot_price[t])*2; //maximum price for up regulation
...
subject to {
  ...
  forall (l in load, t in 2..Ntime)
    // Load consumed
    con3: p_l[l][t] == p_ref[l][t] - p_load_shed[l][t] + changel[l][t];
    
  forall (l in load)
    // Load consumed at time 1
    con3b: p_l[l][1] == p_ref[l][1] - p_load_shed[l][1] + changel[l][1] + phase_3_adjustment[l];
  ...
  forall (g in generators, t in time:(bin[t] == 2)){
    ...
    con19: p_dr[g][t] >= z[g][t]*bid_percent*quant_do[g][t];
  }
...
// Minimum on times  
  ...
  forall (l in load, t in time){
    ...
    con30: changel[l][t] <= changel_max[l][t];
    
    // Minimum flexible load consumption
    con31: changel[l][t] >= changel_min[l][t];
  }
  ...
  forall (g in generators, t in time){
    // Minimum up regulation when not at peak
    con33: p_ur[g][t] >= x[g][t]*ramp_rate*bid_percent*quant_up[g][t];

    // Minimum down regulation when not at peak
    con34: p_dr[g][t] >= z[g][t]*ramp_rate*bid_percent*quant_do[g][t];
    }
    
   forall (l in load, t in time)
    // Maximum load for virtual feeders
    con35: p_l[l][t] <= p_l_max[l];
